#!/usr/bin/env python3
"""
Satellite Attitude Simulation with PID Control (Step 3 and Beyond)

This script simulates a satellite with reaction wheels, controlled by a PID controller.
...
from satelliteEnv import SatelliteEnvWithPID  # PID-based reaction wheel environment


def main():
    parser = argparse.ArgumentParser(description="Satellite Attitude Simulation with PID Control")
    parser.add_argument('--sim_time', type=float, default=100.0, help="Total simulation time (s)")
    ...
    parser.add_argument('--output', type=str, default='satellite_simulation_PID.gif', help="Output filename")
    ...
    env = SatelliteEnvWithPID(sim_time=args.sim_time, dt=args.dt, inertia=args.inertia, I_w=args.I_w)
