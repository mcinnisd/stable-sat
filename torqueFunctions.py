...
def external_torque_impulse(t, amplitude=0.1, duration=0.5):
    return np.array([amplitude, 0.0, 0.0]) if t < duration else np.zeros(3)

def external_torque_sinusoid(t, amplitude=0.1, frequency=1.0):
    return amplitude * np.array([
        np.sin(2 * np.pi * frequency * t),
        np.sin(2 * np.pi * frequency * t + np.pi / 3),
        np.sin(2 * np.pi * frequency * t + 2 * np.pi / 3)
    ])

def external_torque_step(t, amplitude=0.1, step_time=1.0):
    return np.array([amplitude, amplitude, amplitude]) if t >= step_time else np.zeros(3)
...
def wheel_torque_impulse(t, amplitude=0.1, duration=0.5):
    return np.array([amplitude, amplitude, amplitude]) if t < duration else np.zeros(3)

def wheel_torque_sinusoid(t, amplitude=0.1, frequency=1.0):
    return amplitude * np.array([
        np.sin(2 * np.pi * frequency * t),
        np.sin(2 * np.pi * frequency * t + np.pi / 4),
        np.sin(2 * np.pi * frequency * t + np.pi / 2)
    ])
