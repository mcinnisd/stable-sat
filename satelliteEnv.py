...
class SatelliteEnvWithLQR(SatelliteEnvWithWheels):
	...
	def compute_lqr_torque(self):
		# Compute error quaternion: desired * (current)^(-1)
		qd = self.q_desired if np.dot(self.q_desired, self.q) >= 0 else -self.q_desired
		q_error = (R.from_quat(qd) * R.from_quat(self.q).inv()).as_quat()

		# q_error = (R.from_quat(self.q_desired) * R.from_quat(self.q).inv()).as_quat()
		if q_error[3] < 0:
			q_error = -q_error
		# Convert to rotation vector
		error = R.from_quat(q_error).as_rotvec()
		# Wrap error angle to lie within [-pi, pi]
		angle = np.linalg.norm(error)
		if angle > np.pi:
			error = error - 2 * np.pi * (error / angle)
		
		# Check if the current target is reached...
		if np.linalg.norm(error) < self.orientation_error_threshold:
		...
		u = np.zeros(3)
		for i in range(3):
			x_i = np.array([error[i], self.omega[i]])
	...
	def step(self, action=None):
		if self.current_time < self.disturbance_time:
			...
			print(f"Time {self.current_time:.2f}: Disturbance torque applied, tau = {tau}")
		else:
			tau = self.compute_lqr_torque()
			print(f"Time {self.current_time:.2f}: LQR control applied, tau = {tau}")
